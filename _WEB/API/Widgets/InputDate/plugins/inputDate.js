...
class EmicWidgetInputDate extends EmicWidget {
  ...
  connectedCallback() {
    ...
    this.inputDate.style.fontSize = "18px"; // Tamaño de letra de 18px para coincidir con la tabla
    ...
    // Añadimos el nuevo input al shadow DOM
    ...
    this.inputDate.addEventListener("change", (event) => {
      ...
      if (window.inputDateChange)
        inputDateChange(this.getAttribute("id"), newFormat); // Usar el nuevo formato aquí también
    });
  ...
  attributeChangedCallback(name, old, now) {
    ...
    switch (name) {
      case "value":
        ...
        let year = '20' + now.slice(0, 2);   // Asume que estamos en el siglo XXI (años 2000)
